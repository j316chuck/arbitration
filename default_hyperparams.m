function [params] = default_hyperparams()
    ...
    % navigation
    params.start = [4;0.7991;pi/4;0.01]; %end %[0.01; 3.86; pi/1.8; 0.01];%tight  %[-4.01; -4; pi/2; 0.01]; %[-7.36; -3.87; -0.02; 0.01]; %[1.49; 4.40; 1.95; 0.01]; %[-0.41; 3.86; 0.91; 0.01]; %[1.52; 1.23; -1.31; 0.01]; %[4.56; 3.95; -pi/10; 0.01]; %[-4; -1; pi/2; 0.01]; %[-9; -9.5; 0; 0.01]; %[0; -9; 0; 0.01];
