function [params] = default_hyperparams()
    ...
    % navigation
    params.start =  [-4; -4; 0; 0.01]; %[-7.36; -3.87; -0.02; 0.01]; %[1.49; 4.40; 1.95; 0.01]; %[-0.41; 3.86; 0.91; 0.01]; %[1.52; 1.23; -1.31; 0.01]; %[4.56; 3.95; -pi/10; 0.01]; %[-4; -1; pi/2; 0.01]; %[-9; -9.5; 0; 0.01]; %[0; -9; 0; 0.01];
    params.goal = [-4; -8.25; 0; 0.01]; %[3.71; -6.28; -2; 0.01]; %[-0.57; -9.62; -1.83; 0.01]; %[0.89; -6.80; -0.77; 0.01]; %[-9.04; -7.02; -0.12; 0.01]; %[4.76; 0.84; 1.26; 0.01]; %[3, 2.75, pi/2, 0.01]; %[2, 4, pi/2, 0.01];
    ...
    params.horizon = 5;
    % blending params
    params.blending_scheme =  'probabilistic_blend_safety_control_traj'; %'probabilistic_blend_safety_control_traj'; %'switch'; 
